    ...
    <div class="content-wrapper">
      <div class="row">
        <div class="col-lg-12 grid-margin stretch-card">
          <h3 class="card-title float-left"><b>Data Outlet</b></h3>
          <div class="card">
            <div class="card-body">
              ...
              >
                <div class="table align-items-center mb-0">
                  <b-table :items="outlet" :fields="fields">
                    <template v-slot:cell(Aksi)="data">
                      <b-button
                        size="sm"
                        class="btn text-sm font-weight-bold text-xs"
                      ...
                      <b-button
                        size="sm"
                        class="btn text-sm font-weight-bold text-xs"
                        v-on:click="Drop(data.item.id_outlet)"
                      >
    ...
    >
      <form>
        <div class="form-group">
          ...
          <input
            type="text-sm font-weight-bold text-xs"
            name="nama_outlet"
            class="form-control"
            id="nama_outlet"
  ...
  methods: {
    ...
    Save: function () {
      ...
      if (this.action === "insert") {
        axios
          .post(base_url + "/outlet", form, conf)
          .then((response) => {
            this.getData();
        ...
        axios
          .put(base_url + "/outlet/" + this.id_outlet, form, conf)
          .then((response) => {
            this.getData();
            this.message = response.data.message;
            this.$bvToast.show("message");
    ...
    Drop: function (id) {
      ...
      if (confirm("Apakah anda yakin ingin menghapus data ini?")) {
        axios
          .delete(base_url + "/outlet/" + id, conf)
          .then((response) => {
            if (response.data.success == true) {
              this.getData();
            }
